...
- `04_metrics.py` writes `metrics_slices.parquet` (full counts) + `metrics_slices.csv` (shareable) and enriches the tile feed to grain `units:[__ALL__ + canonical ICU slugs]`, `periods:["all","month"]`. Proning stays **month-coarse on the tile by design** (weekly is computed in the slices for the drill-down, but only ~1,854 eligible over ~8y → ≈96% of weeks have <10 eligible, too sparse for a scorecard cell). On a scorecard **week** pick the combiner resolves proning to that week's **containing month** (badge `· Mon YYYY · month`) via the contract §4 fallback, so the tile tracks the timeline without ever exposing a single-week denominator (LPV/SAT/SBT answer weeks exactly).
