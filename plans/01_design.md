...
2. `plateau_pressure_obs ≤ 30` cm H₂O. _(2026-06-02: **physiologic floor** — a plateau below the PEEP it was measured against is impossible (plateau = PEEP + ∆P, ∆P ≥ 0), so such values are invalidated → NaN → dropped from the plateau component, composite, ∆P, and histogram. Bound = the plateau-concurrent PEEP, falling back to current PEEP. Removed ~0.9% of plateau-time of implausible low values; Pplat adherence 86.0% → 85.8%.)_
...
8. **`output/04_lpv_dashboard.html`** _(by `04_dashboard.py`)_ — the single self-contained interactive dashboard (Plotly.js inlined; ~5.3 MB). Four tabs (Vt headline + slider · component breakdown · by-unit/temporal · distributions + Table 1). Two global controls: the **Vt-cutoff slider** (indexes the precomputed grid — no in-browser recompute) and a **time-period selector** (Year + Month dropdowns; All-time default). Period-filtering sums per-month counts in JS for headline/components/per-unit bars/histograms; Table 1 + cohort header swap among 92 precomputed per-period aggregated tables. **Trend granularity follows the period:** all-time = monthly full range; a year zooms the x-axis to that year (per-unit monthly lines); a single month switches to a daily site-wide line (from the `days`/`vtd`/`std` daily payload built off `03_vt_grid_daily_allunits` + `03_daily_unit_summary`). _(2026-06-02: a **Week selector** (ISO weeks of the chosen year, mutually exclusive with Month) gives a **full weekly view** — **every** panel (headline, component breakdown, per-unit bars, histograms, Table 1) reflects the chosen ISO week, slider-aware, from per-week rollups: `03` emits `03_weekly_unit_summary.parquet` + `03_vt_grid_weekly.parquet` (reusing its `summarize`/`grid_counts` bucket machinery, all-severity), and `04` adds per-week histograms + Table 1. Severity is forced to "All" at week grain (the weekly rollups are all-severity, to bound size; ~+1.3 MB → 8.1 MB). The week's daily trend is the site-wide daily line.)_ Panels lazy-draw on tab-show (Plotly renders 0×0 in `display:none`); deep-linkable via `#p-vt|p-comp|p-trend|p-dist`. Vendors `output/_vendor/plotly.min.js` from the `plotly` Python package (offline, no network). Embeds aggregated data + binned histograms only — no raw patient rows. _(2026-06-02: gains a "← CLIF ICU Ventilator QI Bundle" back-link to `index.html`.)_
9. **`output/index.html`** _(by `05_scorecard.py`, 2026-06-02)_ — the **landing page**: a glanceable CLIF-house-style ICU ventilator-QI bundle scorecard, per ICU unit × (ISO-week **or** month, mutually exclusive) (no Plotly; inline-SVG donut + sparkline; ~0.65 MB). Tile artwork from `references/images/*.png` (downscaled + base64-embedded; gitignored; **SVG-icon fallback** when absent). The **LPV tile** (real) headlines **Vt ≤ `SCORECARD_VT_CUTOFF` (default 8) mL/kg** adherence with a donut, denominator (patient-hours/days), a goal bar (`LPV_GOAL`), a 3-segment mini-indicator (Plateau ≤30 · ∆P ≤15 · Vt≤8 in *severe* patients), and a weekly sparkline — and links to artifact #8. SAT / SBT / ARDS-proning / mobilization are **placeholders** for the rest of the bundle. Vt≤8 is recomputed weekly from `02_intervals` (status file is default-6); plateau/∆P reuse `02_patient_day_status`; severe subset joins `02d_severity`. Aggregated counts only.
